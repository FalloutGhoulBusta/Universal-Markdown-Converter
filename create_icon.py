...
def create_simple_icon():
    """Create icon from smoke.png if available, otherwise create a simple icon"""
    if not PIL_AVAILABLE:
        print("PIL (Pillow) not available. Skipping icon creation.")
        print("Install with: pip install Pillow")
        return False

    # Try to use smoke.png first
    if os.path.exists('smoke.png'):
        try:
            print("Found smoke.png, converting to icon...")
            # Open the smoke.png image
            ...
            print("✓ Icon created from smoke.png: icon.ico")
            return True

        except Exception as e:
            print(f"✗ Failed to convert smoke.png: {e}")
            print("Falling back to simple icon...")
    else:
        print("smoke.png not found, creating simple icon...")

    # Fallback: Create a 64x64 image with a blue background
    size = 64
...
if __name__ == "__main__":
    ...
    print("Looking for smoke.png to use as icon...")
